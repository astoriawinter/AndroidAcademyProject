package com.tatiana.rodionova.androidacademyproject.custom
...
class RatingBar @JvmOverloads constructor(
    ...
    private var itemSpacing by Delegates.notNull<Int>()
    ...
    init {
        context.theme.obtainStyledAttributes(attrs, R.styleable.RatingBar, 0, 0).run {
            try {
                itemSpacing = getDimensionPixelSize(R.styleable.RatingBar_itemSpacing, 0)
            } finally {
                recycle()
            }
        }
    ...
    private fun getBitmapDrawableFromVectorDrawable(drawable: Drawable): BitmapDrawable {
        val bitmap = Bitmap.createBitmap(
            drawable.intrinsicWidth + itemSpacing,
